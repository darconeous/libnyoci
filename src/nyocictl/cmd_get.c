...
static nyoci_cms_t get_timeout;
...
get_response_handler(int statuscode, void* context) {
	...
	if(	((statuscode < COAP_RESULT_200) ||(statuscode >= COAP_RESULT_400))
		...
		if(get_observe && statuscode == NYOCI_STATUS_TIMEOUT) {
			gRet = 0;
		} else {
			gRet = (statuscode == NYOCI_STATUS_TIMEOUT)?ERRORCODE_TIMEOUT:ERRORCODE_COAP_ERROR;
...
resend_get_request(void* context) {
	...
	status = nyoci_outbound_begin(nyoci_get_current_instance(),COAP_METHOD_GET, get_tt);
	...
	if (request_accept_type != COAP_CONTENT_TYPE_UNKNOWN) {
		status = nyoci_outbound_add_option_uint(COAP_OPTION_ACCEPT, request_accept_type);
		require_noerr(status,bail);
	}
...
send_get_request(
	...
	if(get_observe) {
		flags |= NYOCI_TRANSACTION_OBSERVE;
	}
	...
	status = nyoci_transaction_begin(nyoci, &transaction, get_timeout);
...
tool_cmd_get(
	...
	HANDLE_LONG_ARGUMENT("timeout") get_timeout = (nyoci_cms_t)(1000*strtof(argv[++i], NULL));
	...
	if(size_request) {
		char block[] = {1};
		require(send_get_request(nyoci, url, block, 1), bail);
	} else {
		require(send_get_request(nyoci, url, NULL, 0), bail);
	}
